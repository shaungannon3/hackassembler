...
class Code():
    ...
    def translate(self):
        if self.instruction.get_type() == "a":
            ...
            self.dest_dict = {'': 0, 'M': 1, 'D': 2, 'MD': 3, 'A': 4, 'AM': 5, 'AD': 6, 'AMD': 7}
            self.jump_dict = {'': 0, 'JGT': 1, 'JEQ': 2, 'JGE': 3, 'JLT': 4, 'JNE': 5, 'JLE': 6, 'JMP': 7}
    ...
    def c_instruction(self):
        return '111' + '{0:07b}'.format(self.comp_dict[self.instruction.get_comp()])  + '{0:03b}'.format(0 if self.instruction.get_dest() == "" else self.dest_dict[self.instruction.get_dest()]) + '{0:03b}'.format(0 if self.instruction.get_jump() == "" else self.jump_dict[self.instruction.get_jump()])
